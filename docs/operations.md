...
| Bulletin exists but no Indicators | Source has no supported IOC values, false-positive status, excluded values, or unfamiliar IOC paths | Inspect sanitized bulletin fields and add an explicit mapping if appropriate |
| A field shows a redaction/omission marker | Sensitive key, recognized pattern, or unstructured source content | Review the mapping policy; raw credential content is not a supported output |
...
Field preservation and observable extraction are separate. Arbitrary structured Cyble fields already appear in the sanitized bulletin JSON. Add rules to `CYBLE_FIELD_MAP_PATH` only when a legitimate IOC uses a path the generic extractor does not recognize, or when you want a selected safe summary field.
