...
Version **0.4.0** is an integration preview, reviewed on **2026-09-25**. Implementation checks and an offline contract run against the supplied Anomali Feed SDK establish the behavior described below. A live ThreatStream tenant, ingestion credentials, and running schedule were not available for end-to-end acceptance.
...
The local release check passed **82 tests**, including **10 real-SDK contracts**, with no skips. Compilation, CLI help/version, Git whitespace checks, and installed dependency compatibility also passed. Public CI can run the remaining **72 tests** without distributing the proprietary SDK.
